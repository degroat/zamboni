...
class zamboni
{
    ...
    public  function table($table_selector, $nth_table = 1, $row_selector = 'tr', $header_row = TRUE)
    {
        ...
        foreach($table($row_selector) as $row)
        {
            ...
            foreach($row('td, th') as $col)
            {
                if($header_row == TRUE && $row_count == 1)
                {
                    $columns[] = trim($col->getPlainText());
                }
                else
                {
                    $index = !empty($columns[$col_count]) ? strtolower(str_replace(' ', '_', $columns[$col_count])) : 'column_' . ($col_count+1);


                    $item[$index]['html'] = trim($col->getInnerText());
                    $item[$index]['text'] = trim(strip_tags($col->getInnerText()));
                    foreach($col("a") as $url)
                    {
                        if(!isset($item[$index]['urls']))
                        {
                            $item[$index]['urls'] = [];
                        }

                        $u = array();
                        foreach($url->attributes as $attribute => $value) 
                        {
                            $u[$attribute] = $value;
                            if($attribute == 'href')
                            {
                                ...
                                {
                                    parse_str($url_parts['query'], $u['params']);
                                }
                            }
                        }
                        $u['html'] = trim($url->getInnerText());
                        $u['text'] = trim($url->getPlainText());
                        $item[$index]['urls'][] = $u;
                    }
